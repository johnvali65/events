...
$(document).ready(function() { 
	...
	proQty.on('click', '.qtybtn', function () {
        ...
        var strHtml = '<span class="btn-apply" id="selected_seats"></span>';
    ...
    $('#checkout').submit(function(e) {
    	...
    	if (checkboxes.filter(':checked').length == 0) {
    		Swal.fire({
              icon: 'error',
              title: 'Oops...',
              text: 'Please Choose Seats'
            })
    	} else if (checkboxes.filter(':checked').length != quantity) {
            Swal.fire({
              icon: 'error',
              title: 'Oops...',
              text: 'Please Choose Seats of count ' + quantity
            })
        } else if (checkboxes.filter(':checked').length == quantity) {
	        $("#loader").fadeIn();
	        ...
	        $.post(url, formData, function(response) { // send; response.data will be what is returned
	            if (response.status == 'invalid') {
	                ...
	                Swal.fire({
                    	...
                    	window.location.replace(base_url+'/my-tickets');
                    });
	            }
	        })
    	}
			...
			success: function(result){				
				if(result.success==true){
					...
					if(result.data.ticket.type=="free"){
						...
						$('.order-bottom').append('<div class="order-ticket-detail mb-4"><div><p>'+result.data.ticket.name+'</p></div><div> '+result.data.quantity+' * '+result.data.ticket.price+'</div></div><hr><div class="order-total"><p>Category</p><p>'+result.data.category.toUpperCase()+'</p></div><div class="order-total"><p>Seats</p><p>'+result.data.seats+'</p></div><hr><div class="order-total"> <p>Ticket Price</p><p> '+(result.data.ticket.price*result.data.quantity)+'</p></div><div class="order-total"> <p>Coupon discount</p><p>(-) '+result.data.coupon_discount+'</p></div><div class="order-total"><p>Tax</p><p>(+) '+result.data.tax+'</p></div><div class="order-total"> <h6>Total</h6><h6>'+result.data.payment+'</h6></div>');
					}	
					if(result.data.review!=null){					
						for ( i = 1; i <= 5; i++) {							
