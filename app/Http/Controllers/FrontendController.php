...
class FrontendController extends Controller
{
    ...
    public function checkout($id){
        ...
        $silver_booked_seats = Order::where([['order_status','Pending']])
                                            ->where('ticket_id', $data->id)
                                            ->where('seats','<>',null)
                                            ->orderBy('id','DESC')
                                            ->where('category','silver')
                                            ->pluck('seats');
        $gold_booked_seats = Order::where([['order_status','Pending']])
                                            ->where('ticket_id', $data->id)
                                            ->where('seats','<>',null)
                                            ->orderBy('id','DESC')
                                            ->where('category','gold')
                                            ->pluck('seats');
        $vip_booked_seats = Order::where([['order_status','Pending']])
                                            ->where('ticket_id', $data->id)
                                            ->where('seats','<>',null)
                                            ->orderBy('id','DESC')
                                            ->where('category','vip')
        ...
        $gold_seats = array();
        $vip_seats = array();
        foreach ($silver_booked_seats as $key => $value) {
            // return $value;
            $x = explode(',', $value);
            foreach($x as $d){
                $silver_seats[] = $d;
        ...
        foreach ($gold_booked_seats as $key => $value) {
            // return $value;
            $x = explode(',', $value);
            foreach($x as $d){
                $gold_seats[] = $d;
        ...
        foreach ($vip_booked_seats as $key => $value) {
            // return $value;
            $x = explode(',', $value);
            foreach($x as $d){
                $vip_seats[] = $d;
        ...
        $data->silver_booked_seats = $silver_seats;
        $data->gold_booked_seats = $gold_seats;
        $data->vip_booked_seats = $vip_seats;
    ...
    public function createOrder(Request $request){
        ...
        return response()->json(['status' => 'valid', 'message' => 'Booking Successfull.']);  
    }
